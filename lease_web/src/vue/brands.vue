...
const checkData = async (val) => {
  ...
  // 查询数据
  let result = await api.getParams("/api/brands/page", checkObj.value)
...
const deleteByIds = () => {
  ...
  ElMessageBox.confirm('确认删除吗？', '提示', {
    ...
    let result = await api.delByIds("/api/brands", listSelectId);
    if (result.code === 200) {
      checkData();
...
const updateDate = () => {
  ...
  if(!editObject.value.url || editObject.value.url == ""){
    // 图片地址为空，不发送请求
...
const addEditSave = async () => {
  ...
  let result = await api.putJson("/api/brands", editObject.value)
  ...
  if (result.code === 200) {
    ...
    ElMessage({
      type: 'success',
      message: '修改成功：修改行数为' + result.data
    })
  } else {
    ElMessage({
      type: 'warning',
...
//   选中的品牌id
  ...
  // 开始保存
  let result = await api.postJson("/api/brands", addObject.value)
...
<!-- 搜素框 -->
  ...
  <div style="margin-top: 15px;display: flex; column-gap: 20px;margin-bottom: 50px;">
    ...
    <div style="margin-right: 30px;margin-left: 140px;">
      <el-input clearable="true" v-model="checkObj.plateNumber" style="width: 160px" size="large" placeholder="品牌名称" />
    </div>

    <div style="margin-right: 30px;margin-left: 140px;">
      <el-input clearable="true" v-model="checkObj.plateNumber" style="width: 160px" size="large" placeholder="所属国家" />
    </div>

    <div style="margin-right: 30px; margin-left: 140px;">
  <el-date-picker v-model="checkObj.foundedYear" type="date" size="large" placeholder="成立时间" :size="size"
    value-format="YYYY-MM-DD" style="width: 200px" />
    </div>
    <div style="margin-right: 30px;margin-left: 120px;">
    <el-date-picker v-model="checkObj.updateTime" type="date" size="large" placeholder="更新时间" :size="size"
...
<el-tooltip placement="top" content="添加品牌" effect="customized">
  <el-button type="primary" @click="insertDate()" round :icon="Plus" style="width: 80px;" />
</el-tooltip>
<el-tooltip placement="top" content="删除品牌" effect="customized">
  <el-button type="danger" @click="deleteByIds()" round :icon="Minus" style="width: 80px;" />
</el-tooltip>
<el-tooltip placement="top" content="修改品牌" effect="customized">
  <el-button type="warning" @click="updateDate()" round :icon="Edit" style="width: 80px;" />
</el-tooltip>
<el-tooltip placement="top" content="查询品牌" effect="customized">
...
<!-- 按钮 -->
  ...
  <el-table :data="tableData" style="width: 100%" border highlight-current-row="true" @selection-change="selectL"
    ...
    <el-table-column prop="foundedYear" label="成立时间" align="center" />
  ...
  <el-dialog v-model="addWinvisible" title="新增品牌" width="500" align-center center draggable :show-close="false"
    :destroy-on-close="true">
    <el-form style="max-width: 600px" :model="addObject" :size="formSize" status-icon>


        <el-form-item label="品牌LOGO：" :label-width="120" >
        <el-upload
      ...
      <el-form-item label="所属国家：" :label-width="120">
        <el-input v-model="addObject.country" autocomplete="off" />
      </el-form-item>

      <el-form-item label="创建时间：" :label-width="120">
        <el-date-picker v-model="addObject.foundedYear" type="date" placeholder="选择日期" value-format="YYYY-MM-DD" />
      </el-form-item>
  ...
  <el-dialog v-model="editWinvisible" title="品牌编辑" width="500" align-center center draggable :show-close="false"
    :destroy-on-close="true">
    <el-form style="max-width: 600px" :model="editObject" :size="formSize" status-icon>
      ...
      <el-form-item label="所属国家：" :label-width="120">
        <el-input v-model="editObject.country" autocomplete="off" />
      </el-form-item>

      <el-form-item label="创建时间：" :label-width="120">
        <el-date-picker v-model="editObject.foundedYear" disabled type="date" placeholder="选择日期" value-format="YYYY-MM-DD" />
      </el-form-item>
