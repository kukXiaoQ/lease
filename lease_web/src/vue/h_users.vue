...
import { Delete, Plus,RefreshLeft, Edit, Minus, Search, Share, Upload } from '@element-plus/icons-vue'
...
const checkData = async (val) => {
  ...
  // 查询数据
  let result = await api.getParams("/api/users/is", checkObj.value)
...
ElMessageBox.confirm('确认恢复吗？', '提示', {
  ...
  let result = await api.delByIds("/api/users/n_is", listSelectId);
  if (result.code === 200) {
    checkData();
...
const deleteByIds = () => {
  ...
  ElMessageBox.confirm('确认永久删除吗？', '提示', {
    ...
    let result = await api.delByIds("/api/users/is", listSelectId);
    if (result.code === 200) {
      checkData();
