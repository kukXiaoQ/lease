...
import { ElMessage, ElMessageBox,ElNotification,UploadInstance   } from 'element-plus'
...
const checkData = async (val) => {
  ...
  // 查询数据
  let result = await api.getParams("/api/system-logs", checkObj.value)
...
const deleteByIds = () => {
  ...
  ElMessageBox.confirm('确认删除吗？', '提示', {
    ...
    let result = await api.delByIds("/api/system-logs", listSelectId);
    if (result.code === 200) {
...
const exportData =async () => {
  ...
  // 导出数据
  let result=await api.get("/api/loading/up_logs");
...
<div style="margin-right: 30px;margin-left: 100px;">
  <el-input v-model="checkObj.username" style="width: 160px" clearable size="large" placeholder="操作用户" />
</div>
<div style="margin-right: 30px;margin-left: 140px;">
  <el-input v-model="checkObj.nickname" style="width: 160px" clearable size="large" placeholder="手机号" />
</div>
<div style="margin-right: 30px;margin-left: 140px;">
  <el-input v-model="checkObj.action" style="width: 160px" clearable size="large" placeholder="操作描述" />
</div>
<div style="margin-right: 30px;margin-left: 140px;">
  <el-date-picker v-model="checkObj.localDate" type="date" clearable size="large" placeholder="操作时间" 
  ...
  <el-table :data="tableData" style="width: 100%" border highlight-current-row="true" @selection-change="selectL"
    ...
    <el-table-column prop="username" label="操作用户" width="180" align="center"  />
    <el-table-column prop="phone" label="手机号" width="160" align="center"  />
    <el-table-column prop="action" label="操作描述" width="200" align="center"  />
    <el-table-column prop="details" label="操作详情" align="center"  />
    <el-table-column prop="time" type="date" width="240" label="操作时间" align="center"  />
  </el-table>
