...
import { Delete, RefreshLeft, Edit,Minus, Search, Share, Upload } from '@element-plus/icons-vue'
const tableData = ref([]);
...
const checkData = async (val) => {
  ...
  // 查询数据
  let result = await api.getParams("/api/rentals/is", checkObj.value)
...
const recoverDate = () => {
  ...
  ElMessageBox.confirm('确认恢复吗？', '提示', {
    ...
    let result = await api.delByIds("/api/rentals/n_is", listSelectId);
    if (result.code === 200) {
...
const deleteByIds = () => {
  ...
  ElMessageBox.confirm('确认永久删除吗？', '提示', {
    ...
    let result = await api.delByIds("/api/rentals/is", listSelectId);
    if (result.code === 200) {
