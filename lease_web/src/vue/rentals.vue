...
const checkData = async (val) => {
  ...
  // 查询数据
  let result = await api.getParams("/api/rentals", checkObj.value)
...
const deleteByIds = () => {
  ...
  ElMessageBox.confirm('确认删除吗？', '提示', {
    ...
    let result = await api.delByIds("/api/rentals", listSelectId);
    if (result.code === 200) {
...
const addEditSave = async () => {
    ...
    let result = await api.putJson("/api/rentals", editObject.value)
...
const addOrderSave = async () => {
  ...
  // 开始保存
  let result = await api.postJson("/api/rentals",addObject.value)
...
<div style="margin-right: 30px;margin-left: 10px;">
    ...
    <el-date-picker v-model="checkObj.rentTime" type="date" size="large" placeholder="租车时间" :size="size"
    value-format="YYYY-MM-DD" style="width: 200px" />
</div>
<div style="margin-right: 30px;margin-left: 10px;">
    <el-date-picker v-model="checkObj.returnTime" type="date" size="large" placeholder="还车时间" :size="size"
    value-format="YYYY-MM-DD" style="width: 200px" />
</div>
...
<div style="margin-right: 30px;">
  ...
  <el-tooltip placement="top" content="添加租赁记录" effect="customized">
    <el-button type="primary" @click="insertDate()" round :icon="Plus" style="width: 80px;" />
  </el-tooltip>
  <el-tooltip placement="top" content="删除租赁记录" effect="customized" >
    <el-button type="danger" @click="deleteByIds()" round :icon="Minus" style="width: 80px;"/>
  </el-tooltip>
  <el-tooltip placement="top" content="修改租赁记录"  effect="customized">
    <el-button type="warning" @click="updateDate()" round :icon="Edit" style="width: 80px;"/>
  </el-tooltip>
  <el-tooltip placement="top" content="查询租赁记录" effect="customized" >
  ...
  <el-table :data="tableData" style="width: 100%" border highlight-current-row="true" @selection-change="selectL"
    ...
    <el-table-column prop="rentTime" label="租车时间" align="center"  />
    <el-table-column prop="preReturnTime" label="预计还车时间" align="center"  />
  ...
  <el-form-item label="租车时间：" :label-width="120">
    <el-date-picker v-model="editObject.rentTime" type="date" placeholder="选择日期" value-format="YYYY-MM-DD" />
  </el-form-item>
  <el-form-item label="预计还车时间：" :label-width="120">
    <el-date-picker v-model="editObject.preReturnTime" type="date" placeholder="选择日期" value-format="YYYY-MM-DD" />
  </el-form-item>
    <el-form-item label="总费用：" :label-width="120">
        <el-input v-model="editObject.totalFee" autocomplete="off" />
  </el-form-item>
...
<el-dialog 
  ...
  <el-form-item label="租车时间：" :label-width="120">
    <el-date-picker v-model="addObject.rentTime" type="date" placeholder="选择日期" value-format="YYYY-MM-DD" />
  </el-form-item>
  <el-form-item label="预计还车时间：" :label-width="120">
    <el-date-picker v-model="addObject.preReturnTime" type="date" placeholder="选择日期" value-format="YYYY-MM-DD" />
  </el-form-item>
    <el-form-item label="总费用：" :label-width="120">
        <el-input v-model="addObject.totalFee" autocomplete="off" />
  </el-form-item>
  <el-form-item label="创建时间：" :label-width="120">
