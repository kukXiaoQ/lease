...
const checkData = async (val) => {
  ...
  // 查询数据
  let result = await api.getParams("/api/orders", checkObj.value)
...
const deleteByIds = () => {
  ...
  ElMessageBox.confirm('确认删除吗？', '提示', {
    ...
    let result = await api.delByIds("/api/orders", listSelectId);
    if (result.code === 200) {
      checkData();
...
const addEditSave = async () => {
  ...
  let result = await api.putJson("/api/orders", editObject.value)
...
const addOrderSave = async () => {
  ...
  // 开始保存
  let result = await api.postJson("/api/orders", addObject.value)
...
<template>
  ...
  <div style="margin-top: 15px;display: flex; column-gap: 40px;margin-bottom: 50px;">
    ...
    <div style="margin-right: 30px;margin-left: 10px;">
      <el-input v-model="checkObj.phone" style="width: 160px" clearable size="large" placeholder="手机号码" />
    </div>

    <div style="margin-right: 30px;margin-left: 10px;">
      <el-input v-model="checkObj.plateNumber" style="width: 160px" clearable size="large" placeholder="车牌号码" />
    </div>

    <div  style="margin-right: 30px;margin-left: 10px;">
<el-select v-model="checkObj.status" placeholder="订单状态" clearable style="width: 160px" size="large">
    ...
    <div style="margin-right: 30px;">
      <el-date-picker v-model="checkObj.startTime" type="date" clearable size="large" placeholder="开始时间" :size="size"
  ...
  <div style="margin-bottom: 30px;display: flex; justify-content: center; align-items: center; column-gap: 50px;">
    ...
    <el-tooltip placement="top" content="添加订单" effect="customized">
      <el-button type="primary" @click="insertDate()" round :icon="Plus" style="width: 80px;" />
    </el-tooltip>
    <el-tooltip placement="top" content="删除订单" effect="customized">
      <el-button type="danger" @click="deleteByIds()" round :icon="Minus" style="width: 80px;" />
    </el-tooltip>
    <el-tooltip placement="top" content="修改订单" effect="customized">
  ...
  <el-table :data="tableData" style="width: 100%" border highlight-current-row="true" @selection-change="selectL"
    ...
    <el-table-column prop="id" label="订单编号" align="center" width="200" />
    <el-table-column prop="username" label="用户名" align="center" />
    <!-- <el-table-column prop="nickname" label="用户昵称" align="center" /> -->
    <el-table-column prop="phone" label="手机号码" align="center" />
    <el-table-column prop="plateNumber" label="车牌号码" align="center" />
    <el-table-column prop="status" label="订单状态" align="center" />
    <el-table-column prop="actualRentalPrice" label="实租价格" align="center" />
    <el-table-column prop="deposit" label="押金" align="center" />
    <el-table-column prop="startTime"  label="订单开始时间" align="center" />
  </el-table>
  ...
  <el-dialog v-model="editWinvisible" title="订单编辑" width="500" align-center center draggable :show-close="false"
    ...
    <el-form style="max-width: 600px" :model="editObject" :size="formSize" status-icon>
      ...
      <el-form-item label="车牌号码：" :label-width="120">
        <el-input v-model="editObject.plateNumber" disabled autocomplete="off" />
      </el-form-item>

      <el-form-item label="实租价格：" :label-width="120">
        <el-input v-model="editObject.actualRentalPrice" autocomplete="off" />
      </el-form-item>
      <el-form-item label="押金：" :label-width="120">
        <el-input v-model="editObject.deposit" autocomplete="off" />
      </el-form-item>

      <!-- 设置字体大小 -->
      <el-form-item label="订单状态：" :label-width="120">
        <el-select v-model="editObject.status" placeholder="please select your zone">
      ...
      <el-form-item label="订单开始时间：" :label-width="120">
        <el-date-picker v-model="editObject.startTime" disabled type="date" placeholder="选择日期" value-format="YYYY-MM-DD" />
      </el-form-item>
      <el-form-item label="订单结束时间：" :label-width="120">
        <el-date-picker v-model="editObject.endTime" disabled type="date" placeholder="选择日期" value-format="YYYY-MM-DD" />
      </el-form-item>
      <el-form-item label="订单更新时间：" :label-width="120">
  ...
  <el-dialog v-model="addWinvisible" title="新增订单" width="500" align-center center draggable :show-close="false"
    :destroy-on-close="true">
    <el-form style="max-width: 600px" :model="addObject" :size="formSize" status-icon>

      <el-form-item label="用户ID：" :label-width="120">
        <el-input v-model="addObject.userId" autocomplete="off" />
      </el-form-item>
      <el-form-item label="车辆ID：" :label-width="120">
        <el-input v-model="addObject.vehicleId" autocomplete="off" />
      </el-form-item>

      <el-form-item label="实租价格：" :label-width="120">
        <el-input v-model="addObject.actualRentalPrice" autocomplete="off" />
      </el-form-item>
      <el-form-item label="押金：" :label-width="120">
        <el-input v-model="addObject.deposit" autocomplete="off" />
      </el-form-item>

      <!-- 设置字体大小 -->
      <el-form-item label="订单状态：" :label-width="120">
        <el-select v-model="addObject.status" placeholder="请选择订单状态">
          <el-option label="待确认" value="0" />
          <el-option label="已确认" value="1" />
          <el-option label="已取消" value="2" />
          <el-option label="已完成" value="3" />
      ...
      <el-form-item label="订单开始时间：" :label-width="120">
        <el-date-picker v-model="addObject.startTime" type="date" placeholder="选择日期" value-format="YYYY-MM-DD" />
      </el-form-item>
      <el-form-item label="订单结束时间：" :label-width="120">
        <el-date-picker v-model="addObject.endTime" type="date" disabled  placeholder="选择日期" value-format="YYYY-MM-DD" />
      </el-form-item>
      <el-form-item label="订单更新时间：" :label-width="120">
